...
class Doctor {
  ...
  static async getAll() {
    try {
      const query = `
                SELECT 
                    m.medico_id, m.medico_estado, m.medico_consultorio,
                    u.usuario_nombre, u.usuario_apellido, u.usuario_correo, 
                    u.usuario_telefono, u.usuario_identificacion, u.usuario_foto_perfil,
                    e.especialidad_nombre, e.especialidad_id
                FROM medicos m
  ...
  static async getById(medico_id) {
    try {
      const query = `
                SELECT 
                    m.medico_id, m.usuario_id, m.especialidad_id, m.medico_estado, m.medico_consultorio,
                    u.usuario_nombre, u.usuario_apellido, u.usuario_correo, 
                    u.usuario_telefono, u.usuario_identificacion, u.usuario_direccion,
                    u.usuario_ciudad, u.usuario_edad, u.usuario_genero, u.usuario_foto_perfil,
                    e.especialidad_nombre, e.especialidad_id
                FROM medicos m
                INNER JOIN usuarios u ON m.usuario_id = u.usuario_id
                LEFT JOIN especialidades e ON m.especialidad_id = e.especialidad_id
  ...
  static async update(medico_id, medicoData) {
    try {
      ...
      if (medicoData.especialidad_id !== undefined) {
        fields.push("especialidad_id = ?");
        values.push(medicoData.especialidad_id);
      }
      if (medicoData.medico_consultorio !== undefined) {
        fields.push("medico_consultorio = ?");
        values.push(medicoData.medico_consultorio);
      }
      if (medicoData.medico_estado !== undefined) {
        fields.push("medico_estado = ?");
        values.push(medicoData.medico_estado);
      }
      ...
      const query = `UPDATE medicos SET ${fields.join(", ")} WHERE medico_id = ?`;
      ...
      const updatedDoctor = await this.getById(medico_id);
      return { success: result.data.affectedRows > 0, doctor: updatedDoctor };
    } catch (error) {
      throw new Error(`Error al actualizar médico: ${error.message}`);
    }
  }

  // Actualizar información médica (especialidad, consultorio) - para médicos
  static async updateMedicalInfo(medico_id, medicoData) {
    try {
      const fields = [];
      const values = [];
      ...
      throw new Error(`Error al actualizar información médica: ${error.message}`);
    }
  }
  ...
  static async search(filters = {}) {
    try {
      let query = `
        SELECT 
          m.medico_id, m.medico_consultorio, m.medico_estado,
          u.usuario_nombre, u.usuario_apellido, u.usuario_correo, 
          u.usuario_telefono, u.usuario_identificacion, u.usuario_foto_perfil,
          e.especialidad_nombre, e.especialidad_id
        FROM medicos m
      ...
      if (filters.especialidad_id) {
        query += " AND m.especialidad_id = ?";
        params.push(filters.especialidad_id);
      }

      if (filters.nombre) {
        query += " AND (u.usuario_nombre LIKE ? OR u.usuario_apellido LIKE ?)";
        const searchTerm = `%${filters.nombre}%`;
        params.push(searchTerm, searchTerm);
      }

      if (filters.fecha && filters.hora) {
        // Buscar médicos con disponibilidad en fecha y hora específica
        query += ` AND EXISTS (
          SELECT 1 FROM disponibilidad d 
          WHERE d.medico_id = m.medico_id 
          AND d.disponibilidad_fecha = ? 
          AND d.disponibilidad_hora = ?
          AND d.disponibilidad_estado = 1
        )`;
        params.push(filters.fecha, filters.hora);
      } else if (filters.fecha) {
        // Buscar médicos con disponibilidad en fecha
        query += ` AND EXISTS (
          SELECT 1 FROM disponibilidad d 
          WHERE d.medico_id = m.medico_id 
          AND d.disponibilidad_fecha = ?
      ...
      query += " ORDER BY u.usuario_nombre, u.usuario_apellido";
