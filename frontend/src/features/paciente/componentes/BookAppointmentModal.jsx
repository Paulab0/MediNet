...
const BookAppointmentModal = ({ isOpen, onClose, doctor, onSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen && doctor?.medico_id) {
      // Resetear formulario
      setSelectedDate("");
      setSelectedTime("");
      setFormData({ cita_tipo: "Consulta", cita_observaciones: "" });
      setError(null);
      setSuccess(null);
      setAvailableSlots([]);
    }
  ...
  useEffect(() => {
    if (selectedDate && doctor?.medico_id) {
      loadAvailableSlots(selectedDate);
    } else {
      setAvailableSlots([]);
      setSelectedTime("");
    }
  }, [selectedDate, doctor?.medico_id]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user?.paciente_id) {
      setError("No se encontró tu información de paciente. Por favor, inicia sesión nuevamente.");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        setSuccess("¡Cita agendada exitosamente! Recibirás un email de confirmación.");
        setTimeout(() => {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {doctor && `Dr. ${doctor.usuario_nombre} ${doctor.usuario_apellido}`}
              </p>
            </div>
          </div>
          <button
