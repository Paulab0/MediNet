...
const EditMedicalHistoryForm = ({ isOpen, onClose, patient, record, onSuccess }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (isOpen && record) {
      setFormData({
        historial_tipo: record.historial_tipo || "Consulta",
        historial_diagnostico: record.historial_diagnostico || "",
        historial_tratamiento: record.historial_tratamiento || "",
        historial_observaciones: record.historial_observaciones || "",
        historial_medicamentos: record.historial_medicamentos || "",
        historial_proxima_cita: record.historial_proxima_cita
          ? new Date(record.historial_proxima_cita).toISOString().split("T")[0]
          : "",
        historial_estado_paciente: record.historial_estado_paciente || "Estable",
      });
    }
  }, [isOpen, record]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await historialService.updateHistoryRecord(record.historial_id, {
        ...formData,
      ...
      setError(
        ...
        err.message ||
        "Error al actualizar el historial médico. Por favor, intenta nuevamente."
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Editar Historial Médico</h2>
              {patient && (
        ...
        {/* Form - Reutilizar la misma estructura que MedicalHistoryForm */}
