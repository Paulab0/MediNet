...
const AddPatientForm = ({ isOpen, onClose, onPatientAdded }) => {
    ...
    switch (name) {
      ...
      case "usuario_edad":
        if (!value.trim()) {
          ...
          error = "La edad debe estar entre 1 y 129 años";
        }
        break;
      case "usuario_genero":
        if (!value) {
  ...
  const handleChange = (e) => {
    ...
    // Limpiar error de submit cuando el usuario modifica cualquier campo
    if (errors.submit) {
  ...
  const handleBlur = (e) => {
    ...
    // Limpiar error de submit cuando el usuario interactúa con campos
    if (errors.submit) {
      setErrors((prev) => ({ ...prev, submit: "" }));
    }
  };
  ...
  const handleFocus = (e) => {
    ...
    // Limpiar error de submit cuando el usuario hace focus en campos
    if (errors.submit) {
      setErrors((prev) => ({ ...prev, submit: "" }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Generar una contraseña temporal automática
      ...
      // Detectar errores de duplicidad y mostrar mensaje específico
      let errorMessage = error.message || "Error al crear el paciente";
      ...
      if (error.message && error.message.toLowerCase().includes("duplicate")) {
        if (
          ...
          errorMessage =
            "El correo electrónico ya está registrado. Por favor, usa un correo diferente.";
        } else if (
          error.message.toLowerCase().includes("identificacion") ||
          error.message.toLowerCase().includes("identification")
        ) {
          errorMessage =
            "El número de identificación ya está registrado. Por favor, verifica el número.";
        } else {
          errorMessage =
            "Los datos ingresados ya existen en el sistema. Por favor, verifica la información.";
        }
      }
            ...
            {errors.submit && (
              <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl">
                ...
                <p className="text-sm text-red-600 mt-2">
                  Corrige la información y vuelve a intentar.
                </p>
              </div>
            )}
